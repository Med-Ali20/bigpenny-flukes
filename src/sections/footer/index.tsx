    ...
    <footer
      className={`${
        path === "/" || path === "/book-now/"
          ...
          : "mt-6 bg-primary text-secondary"
      } font-salford flex flex-col text-center justify-center items-center  relative pb-[60px] lg:pb-8 pt-[40px] lg:pt-[80px] text-[#FBFBF8] font-['Salford_Sans'] relative`}
    >
      {circles.map((index) => (
        <span
          key={index}
          className={`${path === "/" || path === "/book-now/" ? 'bg-secondary' : 'bg-primary'} absolute -top-10 rounded-full w-[5rem] h-[5rem]`}
      ...
      <h2 className="font-black uppercase text-[80px] leading-[70px] mb-[15px] lg:mb-6 lg:text-[150px] lg:leading-36">
        <span className="">
          <Image
            src={`${path === "/" || path === "/book-now/" ? '/flukes/illustrations/logo-horizontal.png' : '/flukes/illustrations/logo-horizontal-white.svg'}`}
          ...
          />
          <BookNowRectangular
            className={`${path === "/" || path === "/book-now/" ? 'bg-primary text-secondary' : 'bg-secondary text-primary' }  text-[2rem] mt-8 mb-4 w-fit mx-auto h-auto leading-0 py-[2rem] px-[2rem]`}
          />
        </span>
      </h2>
